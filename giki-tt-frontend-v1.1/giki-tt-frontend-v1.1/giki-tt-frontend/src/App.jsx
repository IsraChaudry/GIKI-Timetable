...
import { auth, faculties, departments, batches, teachers, rooms, courses, terms } from './lib/api.js'
...
import { RoomSchedule, TeacherSchedule, DeptTimetable, FacultyTimetable } from './pages/Schedules.jsx'
import {
...
const ENTITIES = { faculties, departments, batches, teachers, rooms, courses, terms }

function AppInner() {
  const [authed, setAuthed] = useState(auth.isAuthed())
  const [page, setPage] = useState('dashboard')
  const [data, setData] = useState({ faculties: null, departments: null, batches: null, teachers: null, rooms: null, courses: null, terms: null })
  const [bootLoading, setBootLoading] = useState(false)
  const [theme, setTheme] = useState(() => localStorage.getItem('giki-theme') || 'dark')
  ...
  const handleLogout = () => {
    auth.logout()
    setAuthed(false)
    setData({ faculties: null, departments: null, batches: null, teachers: null, rooms: null, courses: null, terms: null })
    setPage('dashboard')
  }
  ...
  return (
    <Layout page={page} setPage={setPage} onLogout={handleLogout} theme={theme} toggleTheme={toggleTheme}>
      ...
      {page === 'faculty-timetable' && <FacultyTimetable data={data} />}
      {page === 'dept-timetable' && <DeptTimetable    data={data} />}
      ...
      {page === 'terms'          && <Terms           data={data} refreshOne={refreshOne} />}
    </Layout>
  )
}
