import {
  faculties, departments, batches, teachers, rooms, courses, terms,
  uploadCourses,
} from '../lib/api.js'
  ...
  return (
    <EntityTable
      ...
      columns={[
        ...
        { key: 'dept_id', label: 'Department',
          render: (it) => <span className="text-xs text-slate-300">{lookup(data.departments, it.dept_id)}</span> },
        { key: 'unavailable_slots', label: 'Unavailable',
          render: (it) => {
            const slots = it.unavailable_slots || []
            if (!slots.length) return <span className="text-muted text-xs">—</span>
            return <span className="badge-warn">{slots.length} slot{slots.length !== 1 ? 's' : ''}</span>
          }
        },
      ]}
      blank={{ name: '', dept_id: '', unavailable_slots: [] }}
      fields={[
        { key: 'name',              label: 'Name',       type: 'text',       required: true },
        { key: 'dept_id',           label: 'Department', type: 'select-num', required: true,
          options: data.departments?.map((d) => ({ value: d.id, label: d.name })) },
        // The unavailable_slots field uses a custom JSON-string editor since EntityTable's
        // generic field types don't cover compound list inputs. Power users can type JSON
        // directly; the API expects e.g. [{"day":"Monday","slot":0}, {"day":"Friday","slot":5}]
        { key: 'unavailable_slots', label: 'Unavailable slots (JSON list)', type: 'text',
          hint: 'e.g. [{"day":"Monday","slot":0}, {"day":"Friday","slot":5}] — leave blank for full availability.' },
      ]}
      search={(it, q) => it.name?.toLowerCase().includes(q)}
    />
  )
...
export function Courses({ data, refreshOne }) {
  ...
  const handleUpload = async () => {
    if (!file) {
      setUploadError('Choose a CSV/XLS file first.')
      return
    }
    setUploading(true)
    setUploadError('')
    ...
    try {
      ...
      const data = await uploadCourses(formData)
      setUploadMessage(data.message || `Uploaded ${data.inserted} courses.`)
      setFile(null)
    ...
    <>
      <div className="card mb-4 p-4 space-y-3">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <div className="font-medium text-slate-100">Upload courses</div>
            <div className="text-sm text-slate-500">
              Choose a CSV or Excel file with columns: code, section, title, credit_hours,
              type, instructor, program.
            </div>
          </div>
        ...
        {uploadMessage && <Notice kind="success">{uploadMessage}</Notice>}
      </div>
...
export function Terms({ refreshOne }) {
  return (
    <EntityTable
      api={terms}
      title="Term"
      onLoad={(arr) => refreshOne?.('terms', arr)}
      columns={[
        { key: 'id',        label: 'ID',   className: 'font-mono text-muted text-xs' },
        { key: 'code',      label: 'Code',
          render: (it) => <span className="font-mono text-cyan-400">{it.code}</span> },
        { key: 'name',      label: 'Name', className: 'text-slate-200' },
      ...
      blank={{ name: '', code: '', is_active: false }}
      fields={[
        { key: 'name',      label: 'Name', type: 'text', required: true, hint: 'e.g. Fall 2026' },
        { key: 'code',      label: 'Code', type: 'text', required: true, hint: 'e.g. F26' },
        { key: 'is_active', label: 'Set as active term', type: 'checkbox',
          hint: 'Only one term can be active at a time. New schedules are tagged with the active term.' },
      ]}
      search={(it, q) => it.name?.toLowerCase().includes(q) || it.code?.toLowerCase().includes(q)}
    />
  )
