...
async function request(path, opts = {}) {
  ...
  // Browser will set multipart/form-data boundary for FormData bodies automatically.
...
export const terms       = crud('terms')

export async function uploadCourses(formData) {
  return request('/upload/courses', { method: 'POST', body: formData })
}

// ── individual schedule rows (for manual drag/drop edits) ────────────────
export const schedules = {
  get:    (id)       => request(`/schedules/${id}`),
  patch:  (id, data) => request(`/schedules/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  remove: (id)       => request(`/schedules/${id}`, { method: 'DELETE' }),
}

// ── public (no-auth) endpoints ───────────────────────────────────────────
// Useful for student-facing pages that should not require admin login.
export const publicApi = {
  timetable:       (batchId)   => fetch(`${API}/public/timetable/${batchId}`).then(r => r.json()),
  teacherSchedule: (teacherId) => fetch(`${API}/public/teacher/${teacherId}`).then(r => r.json()),
  batches:         ()          => fetch(`${API}/public/batches`).then(r => r.json()),
  teachers:        ()          => fetch(`${API}/public/teachers`).then(r => r.json()),
}
...
export const scheduler = {
  generate:    (scope, id) =>
    request(`/scheduler/generate?scope=${encodeURIComponent(scope)}&id=${encodeURIComponent(id)}`,
            { method: 'POST' }),
  generateAll: () => request('/scheduler/generate-all', { method: 'POST' }),
  ...
  clear:       (batchId, force) => request(`/scheduler/clear/${batchId}?force=${force ? 'true' : 'false'}`,
                                           { method: 'DELETE' }),
  clearAllDrafts: () => request('/scheduler/clear-all-drafts', { method: 'DELETE' }),
}

export const conflicts = {
  get:    (batchId) => request(`/conflicts/${batchId}`),
  global: ()        => request(`/conflicts/global/all`),
}
...
export async function exportFacultyPdf(facultyId, facultyCode) {
  const t = token.get()
  const res = await fetch(`${API}/export/pdf/faculty/${facultyId}`, {
    headers: { Authorization: `Bearer ${t}` },
  })
  if (!res.ok) throw new Error('Faculty PDF export failed')
  const blob = await res.blob()
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a')
  a.href = url
  a.download = `timetable_${facultyCode || facultyId}.pdf`
  document.body.appendChild(a)
  a.click()
  document.body.removeChild(a)
  setTimeout(() => URL.revokeObjectURL(url), 1000)
