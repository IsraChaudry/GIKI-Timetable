...
const NAV_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'dept-timetable',   label: 'Dept Timetable',   icon: '▤' },
...
export function Layout({ page, setPage, children, onLogout, theme, toggleTheme }) {
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="h-14 px-6 border-b border-ink-700 bg-ink-900 flex items-center justify-between flex-shrink-0">
          ...
          <div className="flex items-center gap-4">
            <button
              onClick={toggleTheme}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-ink-700 text-[11px] font-mono text-muted hover:text-slate-200 hover:border-[#429EBD] transition-colors"
              title={theme === 'dark' ? 'Switch to light mode' : 'Switch to dark mode'}
            >
              {theme === 'dark' ? '☀ Light' : '☾ Dark'}
            </button>
            <div className="text-[10px] font-mono text-muted">
              GIK Institute · {new Date().toLocaleDateString('en-PK', { weekday: 'short', day: 'numeric', month: 'short' })}
            </div>
          </div>
        </header>
