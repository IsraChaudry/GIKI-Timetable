...
export function EntityTable({
  ...
  useEffect(() => { reload() /* eslint-disable-next-line */ }, [refreshSignal])
  ...
  const openEdit = (it) => {
    ...
    const data = {}
    fields.forEach((f) => {
      let v = it[f.key] ?? blank[f.key]
      // If the canonical shape is an array, the input is a text field — show as JSON
      if (Array.isArray(blank[f.key]) && Array.isArray(v)) {
  ...
  const save = async () => {
    ...
    try {
      ...
      fields.forEach((f) => {
        ...
        // If the field's blank value is an array, try to coerce string → array via JSON.parse
        if (Array.isArray(blank[f.key]) && typeof v === 'string') {
          if (v.trim() === '') v = []
          else {
            try { v = JSON.parse(v) }
            catch { throw new Error(`Field "${f.label}" must be valid JSON (e.g. [{"day":"Monday","slot":0}])`) }
          }
        }
